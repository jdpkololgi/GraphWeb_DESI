...
def main(args):
    ...
    series = [("Abacus truth", truth, 0.45), ("Regression DESI", reg, 0.7), ("NPE DESI", npe, 1.0)]
    ...
    for i, (lab, frac, alpha) in enumerate(series):
        ...
        ax.bar(x + (i - 1) * w, vals, w, color=cols, alpha=alpha, edgecolor="#F2F2F2", lw=0.7, label=lab)
    ax.set_xticks(x); ax.set_xticklabels([c.capitalize() for c in CLASS_ORDER])
    ax.set_ylabel("Class fraction")
    ax.set_title(rf"DESI T-Web class fractions ($\lambda_{{th}}$={lam_th})")
    ax.legend()
